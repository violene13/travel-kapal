@extends('layouts.adminpelayaran')

@section('title', 'Tambah Jalur Pelayaran')

@section('content')
<div class="container mt-4">
    <h4>Tambah Jalur Pelayaran</h4>
    <div class="card p-3 mt-3 shadow-sm">
        <form action="{{ route('jalurpelayaran.store') }}" method="POST">
            @csrf

            {{-- Pilih Pelabuhan Asal --}}
            <div class="mb-3">
                <label class="form-label">Pelabuhan Asal</label>
                <select name="id_pelabuhan_asal" class="form-select" required>
                    <option value="">-- Pilih Pelabuhan Asal --</option>
                    @foreach ($pelabuhan as $p)
                        <option value="{{ $p->id_pelabuhan }}">{{ $p->nama_pelabuhan }}</option>
                    @endforeach
                </select>
            </div>

            {{-- Pilih Pelabuhan Tujuan --}}
            <div class="mb-3">
                <label class="form-label">Pelabuhan Tujuan</label>
                <select name="id_pelabuhan_tujuan" class="form-select" required>
                    <option value="">-- Pilih Pelabuhan Tujuan --</option>
                    @foreach ($pelabuhan as $p)
                        <option value="{{ $p->id_pelabuhan }}">{{ $p->nama_pelabuhan }}</option>
                    @endforeach
                </select>
            </div>

            {{-- Durasi --}}
            <div class="mb-3">
                <label class="form-label">Durasi</label>
                <input type="text" name="durasi" class="form-control" placeholder="cth: 3 jam" required>
            </div>

            {{-- Jarak --}}
            <div class="mb-3">
                <label class="form-label">Jarak (mil laut / km)</label>
                <input type="number" name="jarak" class="form-control" placeholder="cth: 120" required>
            </div>

            <button type="submit" class="btn btn-success">Simpan</button>
            <a href="{{ route('jalurpelayaran.index') }}" class="btn btn-secondary">Kembali</a>
        </form>
    </div>
</div>
@endsection
